...
const DecreaseStockUseCase = require("../../../../../application/use-cases/Products/DecreaseStockUseCase");

// POST /api/decrease-stock

function createDecreaseStockRouter({ pool }) {
  const router = express.Router();

  router.post("/decrease-stock", async (req, res) => {
    try {
      const productRepository = new PgProductRepository({ pool });
      const useCase = new DecreaseStockUseCase({ productRepository });
...
module.exports = createDecreaseStockRouter;
